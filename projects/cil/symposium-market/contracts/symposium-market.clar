...
(define-constant ERR_NFT_NOT_LISTED_FOR_SALE (err u2002))
(define-constant ERR_NFT_ASSET_MISMATCH (err u2003))
(define-constant ERR_ASSET_NOT_ALLOWED (err u2005))
(define-constant ERR_EXPIRY_IN_PAST (err u106))
...
(define-public (list-in-token (nft-asset-contract <nft-trait>) (token-id uint) (unit-price uint) (amount uint) (expiry uint) (taker (optional principal)) (comm <commission-trait>) (token <ft-trait>))
    (let (
            (listing-id (var-get listing-nonce))
        )
		(asserts! (is-allowed (contract-of nft-asset-contract)) ERR_ASSET_NOT_ALLOWED)
		(asserts! (is-allowed (contract-of token)) ERR_PAYMENT_CONTRACT_NOT_ALLOWED)
		(asserts! (> expiry block-height) ERR_EXPIRY_IN_PAST)
		(asserts! (> unit-price u0) ERR_PRICE_WAS_ZERO)

        (try! (transfer-nft nft-asset-contract token-id amount tx-sender (as-contract tx-sender)))
		(map-set listings listing-id {token-id: token-id, maker: tx-sender, nft-asset-contract: (contract-of nft-asset-contract), expiry: expiry, unit-price: unit-price, amount: amount, taker: taker, commission: (contract-of comm), token: (contract-of token)})
