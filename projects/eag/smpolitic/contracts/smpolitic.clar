...
(define-data-var token-uri (string-ascii 246) "ipfs://QmVXvcdKHUcg1RcsxAASmdAJAJtnxdE4YngcDiuAXcREZN/smpolitic-{id}.json")
...
(define-non-fungible-token smpolitic uint)
...
(define-public (transfer (id uint) (owner principal) (recipient principal))
    (begin
        ...
        (nft-transfer? smpolitic id owner recipient)
    )
)

;; operable
(define-read-only (is-approved (id uint) (operator principal))
    (let ((owner (unwrap! (nft-get-owner? smpolitic id) ERR_COULDNT_GET_NFT_OWNER)))
...
(define-public (mint-with (token <ft-trait>))
    (let (
        ...
        (try! (nft-mint? smpolitic newMintCounter contract-caller))
...
(define-public (admin-mint (recipient principal) (userMintCounter uint))
    (begin
        (asserts! (is-eq contract-caller (var-get ADMIN_MINT_PASS)) ERR_NOT_ADMIN_MINT_PASS)
        (asserts! (<= userMintCounter COLLECTION_MAX_SUPPLY) ERR_COLLECTION_LIMIT_REACHED)
        (asserts! (> userMintCounter u0) ERR_COLLECTION_LIMIT_REACHED)
        (try! (nft-mint? smpolitic userMintCounter recipient))
        (ok userMintCounter)
    )
)

(define-public (admin-mint-many (entries (list 200 {recipient: principal, nftIndex: uint})))
    (fold check-err
        (map admin-mint-next entries)
        (ok true)
    )
)
(define-private (admin-mint-next (entry {recipient: principal, nftIndex: uint}))
    (begin
        (try! (admin-mint (get recipient entry) (get nftIndex entry)))
        (ok true)
    )
)
...
(define-public (list-in-token (id uint) (price uint) (comm <com10>) (token <ft-trait>))
    (let ((listing {price: price, commission: (contract-of comm), token: (contract-of token)})) 
        (asserts! (is-eq contract-caller (unwrap! (nft-get-owner? smpolitic id) ERR_COULDNT_GET_NFT_OWNER)) ERR_NOT_OWNER)
...
(define-public (buy-in-token (id uint) (comm <com10>) (token <ft-trait>))
    (let 
        (
            ...
            (owner (unwrap! (nft-get-owner? smpolitic id) ERR_COULDNT_GET_NFT_OWNER))
        ...
        (try! (nft-transfer? smpolitic id owner buyer))
...
(define-public (burn (id uint))
    (let ((owner (unwrap! (nft-get-owner? smpolitic id) ERR_COULDNT_GET_NFT_OWNER)))
        (asserts! (is-eq owner contract-caller) ERR_NOT_OWNER)
        (map-delete market id)
        (nft-burn? smpolitic id contract-caller)
    )
)
