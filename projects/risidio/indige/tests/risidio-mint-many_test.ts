...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const entries = Array.from({ length: 10 }).map((_, index) => index + 1);
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const entries = Array.from({ length: 20 }).map((_, index) => index + 1);
    let block = chain.mineBlock([
...
Clarinet.test({
  ...
  async fn(chain: Chain, accounts: Map<string, Account>) {
    ...
    const entries1 = Array.from({ length: 10 }).map((_, index) => index + 1);
